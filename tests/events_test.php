...
class assignsubmission_metherpad_events_testcase extends advanced_testcase {
    ...
    /**
     * Setup all the various parts of an assignment activity including creating an metherpad submission.
     */
    protected function setUp() {
        ...
        $this->data->metherpad_editor = array(
    ...
    public function test_assessable_uploaded() {
        ...
        $plugin = $this->assign->get_submission_plugin_by_type('metherpad');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\assignsubmission_metherpad\event\assessable_uploaded', $event);
    ...
    /**
     * Test that the submission_created event is fired when an metherpad submission is saved.
     */
    public function test_submission_created() {
        ...
        $this->assertCount(2, $events);
        $event = $events[1];
        $this->assertInstanceOf('\assignsubmission_metherpad\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    /**
     * Test that the submission_updated event is fired when an metherpad
    ...
    public function test_submission_updated() {
        ...
        $event = $events[3];
        $this->assertInstanceOf('\assignsubmission_metherpad\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
